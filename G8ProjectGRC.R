...
pie_chart <- ggplot(payment_data, aes(x = "", y = count, fill = paymentType, label = paste0(paymentType, ": ", percentage, "%"))) +
...
reordered_data <- within(data, age <- factor(age, levels = age_spending$age))

# Create a barplot using the reordered_data dataframe
bar_plot_age <- ggplot(reordered_data, aes(x = age, y = total)) +
...
num_clusters <- as.integer(readline(prompt = "Enter the number of clusters (2-4): "))
...
min_support <- as.numeric(readline(prompt = "Enter the minimum support (between 0 and 1): "))
min_confidence <- as.numeric(readline(prompt = "Enter the minimum confidence (between 0 and 1): "))

# Check if the input is within the specified range
if (min_support < 0 || min_support > 1 || min_confidence < 0 || min_confidence > 1) {
  stop("Minimum support and confidence must be between 0 and 1.")
}
